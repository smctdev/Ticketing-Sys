...
import { AlertCircleIcon, FileSpreadsheet, Loader2, X } from "lucide-react";
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col gap-4 max-h-[calc(100vh-200px)] overflow-y-auto">
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                {formInput.ticket_support.length + oldFiles?.length > 0
                  ? `Uploaded ${
                      formInput.ticket_support.length + oldFiles?.length
                    } file(s)`
                  : "Upload file"}
              </Button>
              {formInput?.ticket_support?.length + oldFiles?.length > 0 && (
