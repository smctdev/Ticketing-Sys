...
export default function EditTicketModal({
  ...
  const handleRemoveDatabaseFile = async (index) => {
    ...
    if (index < updatedSupport.length) {
      ...
      try {
        ...
        await api.patch(`/updateSupport/${ticketIdToEdit}`, {
  ...
  useEffect(() => {
    async function fetchTicketData() {
      try {
        ...
        const response = await api.get(EDIT_URL);
  ...
  const handleSaveChanges = async (event) => {
    ...
    try {
      if (userRole === 1) {
        ...
        await api.put(SAVE_URL, { userID: selectedUser.value });
      } else {
  ...
  useEffect(() => {
    ...
    async function fetchSupplier() {
      try {
        const response = await api.get("/getAllSupplier");
