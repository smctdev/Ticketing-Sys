...
export default function ViewTicketModal({
  ...
  useEffect(() => {
    async function fetchTicketData() {
      try {
        ...
        const response = await api.get(VIEW_URL);
        ...
        const files = supportFilesData.map((fileUrl) => ({
          ...
          url: `${axios.defaults.baseURL}/uploads/${encodeURIComponent(
            fileUrl
          )}`,
        }));
  ...
  const generatePrintContent = () => {
    ...
    const resappByAutm = appByAutm
      ? `${appByAutm} ${showTicketData?.AutomationHead?.UserDetails?.lname}${
          showTicketData?.appTAutomationHead
            ? ` @ ${showTicketData.appTAutomationHead}`
            : ""
        }`
      : "";
    ...
    const resNoted = [resreqByHead].filter((value) => value !== "");
    const resChecked = [resappByStaff].filter((value) => value !== "");
    const resReview = [resappBySup].filter((value) => value !== "");
    const resApproved = [resappByAutm].filter((value) => value !== "");
    const resEditedBys = [resEditedBy].filter((value) => value !== "");
  ...
  <style>
    ...
    @media print {
      ...
      <div class = "font-style">
        ...
        ${
          resNoted.length > 0
            ? `<div style="display: flex;">Noted by: ${resNoted.join(
                " / "
              )}</div>`
            : ""
        }
        ${
          resReview.length > 0
            ? `<div style="display: flex;">Checked by: ${resChecked.join(
                " / "
              )}</div>`
            : ""
        }
        ${
          resReview.length > 0
            ? `<div style="display: flex;">Reviewed by: ${resReview.join(
                " / "
              )}</div>`
            : ""
        }
        ${
          resApproved.length > 0
            ? `<div style="display: flex;">Approved by: ${resApproved.join(
                " / "
              )}</div>`
            : ""
        }
        ${
          resEditedBys.length > 0
            ? `<div style="display: flex;">Edited by: ${resEditedBys.join(
                " / "
              )}</div>`
            : ""
        }
  ...
  const handleApprove = async (e) => {
    ...
    try {
      ...
      const response = await api.put(UPDATE_STATUS_URL, {
  ...
  const purposes = safeParseJSON(showTicketData?.TicketDetails?.td_purpose);
