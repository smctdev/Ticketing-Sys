  ...
  useEffect(() => {
    const fetchNotif = async () => {
      try {
        ...
        const response = await api.get("/getAllNotifs");
        const accountingNotif = await api.get("/getAssignedCategories");
        ...
        if (userRole === 1) {
          ...
          count = response.data.filter(
            (item) => item.notifAUTM === 1 && item.status === "PENDING"
          ).length;
        }
        setNotifBadge(count);
  ...
  const handleViewTicketClick = async (ticketId, status, displayTicket) => {
    ...
    try {
      ...
      if (userRole === 1 || userRole === 2) {
        await api.put(UPDATE_NOTIF_URL, { notifID: 1 });
      } else if (userRole === 3) {
        await api.put(UPDATE_NOTIF_URL, { notifID: 3 });
      } else if (userRole === 4) {
        await api.put(UPDATE_NOTIF_URL, { notifID: 4 });
      } else if (userRole === 5) {
        await api.put(UPDATE_NOTIF_URL, { notifID: 5 });
      } else if (userRole === 7) {
        await api.put(UPDATE_NOTIF_URL, { notifID: 7 });
      } else if (userRole === 9) {
        await api.put(UPDATE_NOTIF_URL, { notifID: 100 });
      }
                ...
                <MenuHandler>
                  {userProfile.profileImage !== "" ? (
                    <Avatar
                      ...
                      src={`${api.defaults.baseURL}/uploads/${userProfile.profileImage}`}
                    />
                  ) : profileImage === null ? (
                    <Avatar
                      variant="circular"
                      alt="Profile"
                      className="h-10 w-10 cursor-pointer"
                      src={`${api.defaults.baseURL}/uploads/Windows_10_Default_Profile_Picture.svg.png`}
                    />
                  ) : (
                    <Avatar
                      variant="circular"
                      alt="Profile"
                      className="h-10 w-10 cursor-pointer"
                      src={`${api.defaults.baseURL}/uploads/${profileImage}`}
                    />
                  )}
                </MenuHandler>
              </Tooltip>
