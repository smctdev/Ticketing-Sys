...
export function Reports() {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      try {
        const response = await api.get(`/getAllCompleted?${queryParams}`);
    ...
    const fetchBranch = async () => {
      try {
        ...
        setBranchCode(response.data.data);
      } catch (error) {
    ...
    const fetchType = async () => {
      try {
        const response = await api.get("/getAllBranchCategories");
    ...
    <Select
      options={[
        ...
        ...branchCode.map((bcode) => ({
          label: `${bcode?.b_name} (${bcode?.b_code})`,
  ...
  const exportAllToExcel = async () => {
    try {
      ...
      const detailedReports = queryParams
        ? await api.get(`/getReports?${queryParams}`)
        : await api.get("/getReports");
          ...
          const data = {
            ...
            "Requested By": isFirstRow
              ? detailedReport.login_id
                ? capitalizeFirstLetter(
                    detailedReport.UserTicket?.UserDetails?.fname
                  ) +
                  " " +
                  capitalizeFirstLetter(
                    detailedReport.UserTicket?.UserDetails?.lname
                  )
                : ""
              : "",
            "Approve By BM/BS": isFirstRow
              ? detailedReport.approveHead
                ? capitalizeFirstLetter(
                    detailedReport.ApproveHead?.UserDetails?.fname
                  ) +
                  " " +
                  capitalizeFirstLetter(
                    detailedReport.ApproveHead?.UserDetails?.lname
                  )
                : ""
              : "",
            "Approve By Acctg. Staff": isFirstRow
              ? detailedReport.approveAcctgStaff
                ? capitalizeFirstLetter(
                    detailedReport.ApproveAccountingStaff?.UserDetails?.fname
                  ) +
                  " " +
                  capitalizeFirstLetter(
                    detailedReport.ApproveAccountingStaff?.UserDetails?.lname
                  )
                : ""
              : "",
            "Approve By Accounting Head": isFirstRow
              ? detailedReport.approveAcctgSup
                ? capitalizeFirstLetter(
                    detailedReport.ApproveAccountingHead?.UserDetails?.fname
                  ) +
                  " " +
                  capitalizeFirstLetter(
                    detailedReport.ApproveAccountingHead?.UserDetails?.lname
                  )
                : ""
              : "",
            "Edited By": isFirstRow
              ? capitalizeFirstLetter(
                  detailedReport.Automation?.UserDetails?.fname
                ) +
                " " +
                capitalizeFirstLetter(
                  detailedReport.Automation?.UserDetails?.lname
                )
              : "",
