...
function MyProfile() {
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      await api.put("/uploadProfilePic", formData);
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await api.put("/changePass", {
      oldPass,
      newPass,
      confirmPass,
    });
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          const response = await api.put(EDIT_URL, {
                ...
                <div className="flex flex-col">
                    ...
                    <span className="ml-1 flex flex-wrap gap-2">
                      {userData.Branches?.length > 0
                        ? userData.Branches.map((branch, index) => (
                            <span
                              key={index}
                              className="rounded-full border border-gray-400 bg-gray-200 p-2"
                            >
                              {branch.b_name}
                            </span>
                          ))
                  ...
                  <div className="flex items-center">
                    ...
                    <p className="ml-3 -mt-3">
                      {userData.UserDetails?.user_email}
                    </p>
                  </div>
                  <div className="flex items-center">
                    <p className="font-bold">Contact:</p>
                    <p className="ml-3">{userData.UserDetails?.user_contact}</p>
                  </div>
                </div>
              </div>
