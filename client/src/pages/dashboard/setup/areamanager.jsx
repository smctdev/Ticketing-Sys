...
export function SetupAreaManager() {
  ...
  const optionManager = areaManager.map((item) => ({
    value: item.login_id,
  ...
  const handleBranchesChange = (value) => {
    ...
    checkIfAllSelectsHaveValue(managerValue, value);
  };
      ...
      if (result.isConfirmed) {
        ...
        try {
          Swal.fire({
            ...
            html: `Assigning Branch To <strong> ${managerValue.label} </strong> Updated Successfully.`,
            icon: "success",
            confirmButtonColor: "#3085d6",
            confirmButtonText: "Done!",
          });
          await api.post("/assignedManager", {
            managerID: managerValue.value,
  ...
  useEffect(() => {
    const fetchAreaManager = async () => {
      try {
        ...
        setAreaManager(response.data.AreaManagers);
        setBlist(response.data.remainingBranches);
        setIsLoading(true);
      } catch (error) {
        console.log("Error fetching Area Manager:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    const formattedData = areaManager.map((report, key) => [
      String(key + 1),
  ...
  const handleDeleteClick = async (managerID, branchesToDel) => {
    if (branchesToDel.length > 0) {
      ...
      const { isConfirmed } = await Swal.fire({
        ...
        text: `Do you wish to delete all branches assigned from this Area Manager?`,
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
      ...
      if (isConfirmed) {
        try {
          await api.delete("/assignedDeleteManager", {
            data: {
              managerID: managerID,
  ...
  const sortedManager = [...areaManager].sort((a, b) => {
    const nameA =
        ...
        <CardBody className="px-0 pt-0 pb-2">
          <div>
            <Typography variant="h6" color="gray" className="ml-4 pb-2 pt-2">
              Assign Branches to Area Manager
            </Typography>
          </div>
          <hr />
          ...
          <table className="w-full min-w-[640px] table-auto pr-5">
            ...
            <tbody>
              ...
              {isLoading ? (
                ...
                sortedManager.map((item, index) => {
                  const className = `py-3 px-5 ${
                    index === sortedManager.length - 1
                      ? ""
                        ...
                        <Typography className="flex flex-wrap gap-1 text-xs font-normal">
                          ...
                          item.AssignedBranches.length > 0 ? (
                            item.AssignedBranches.map((branch, i) => (
                              <span
                                key={i}
                                className="mr-2 rounded-md bg-blue-500 px-2 py-1 text-white"
                              >
