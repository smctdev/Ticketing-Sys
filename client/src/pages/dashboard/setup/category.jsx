...
export function SetupCategory() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await api.get(`/getAllCategoriesTable?${queryParams}`);
    ...
    fetchCategories();
  }, [searchValue, page, limit, isEditCategoryOpen, isModalOpen, isRefresh]);
  ...
  const handleToggleSwitchChange = (id) => async () => {
    setIsRefresh(true);
    try {
      await api.patch(`/ticket-category/${id}/show-hide`);
    } catch (error) {
      console.error(error);
    } finally {
      setIsRefresh(false);
    ...
    <div className="mt-12 flex flex-col gap-12">
                        ...
                        >
                         <EyeIcon className="w-4 h-4 text-blue-500" title="View"/>
                          ...
                          <button
                            ...
                            onClick={() =>
                              handleEditCategoryClick(item.ticket_category_id)
                            }
                          >
                            <PencilIcon
                          ...
                          <button
                            className="hover:nounderline transform-gpu p-1 text-black transition-transform hover:scale-150"
                            onClick={() =>
                              handleDeleteClick(item.ticket_category_id)
                            }
                          >
                            <TrashIcon
      ...
      {isViewCategoryOpen && (
        <ViewCategoryModal
          ...
          categoryIdToView={showCategoryIdToView}
        />
      )}
      {isEditCategoryOpen && (
        <EditCategoryModal
          ...
          onCloseModal={closeEditModalX}
          categoryIdToEdit={showCategoryIdToEdit}
        />
      )}
    </div>
  );
