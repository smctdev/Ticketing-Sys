...
export function SetupAccounting() {
  ...
  useEffect(() => {
    const fetchAccounting = async () => {
      try {
        const response = await api.get("/getAllAccounting");
        console.log(response.data);
        setAccounting(response.data.accountings);
        setBlist(response.data.group_categories);
        setIsLoading(true);
      } catch (error) {
  ...
  const handleDeleteClick = async (accountingName, branchesToDel) => {
    if (branchesToDel.length > 0) {
      ...
      if (isConfirmed) {
        try {
          await api.delete("/assignedDeleteCategory", {
            data: {
          ...
          <table className="w-full min-w-[640px] table-auto pr-5">
            ...
            <tbody>
              ...
              {isLoading ? (
                ...
                sortedAccounting.map((item, index) => {
                  ...
                  const branches = matchedBranches
                    .map((branch) => branch.CategoryGroup.group_code)
                    .join(", ");
                  const branchesToDel = matchedBranches
                    .map((branch) => branch.CategoryGroup.id)
                    .join(", ");
                  return (
                    <tr className="addbghover">
                      ...
                      <td className={className}>
                        <Typography className="text-xs font-normal">
                          {item.AssignedCategories &&
                          item.AssignedCategories.length > 0 ? (
                            item.AssignedCategories.map((branch, i) => (
                              <span
                                key={i}
                                className="mr-2 rounded-md bg-blue-500 px-2 py-1 text-white"
                              >
                                {branch.category_group_code.group_code}
                              </span>
                            ))
                          ) : (
                      ...
                      <td className={className}>
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            onClick={() =>
                              handleDeleteClick(
                                item.UserDetails.user_details_id,
                                branchesToDel
                              )
                            }
                          >
