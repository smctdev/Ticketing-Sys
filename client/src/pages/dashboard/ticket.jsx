...
export function Ticket() {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      ...
      try {
        const response = await api.get(`/getAllTickets?${queryParams}`);
  ...
  //         `/getAssignedCategoryGroup/${user?.login_id}`
  //       );
    ...
    <Select
      options={[
        ...
        ...branchCode.map((bcode) => ({
          label: `${bcode?.b_name} (${bcode?.b_code})`, // Display both name and code
  ...
  return (
    <div className="mt-12 mb-8 flex flex-col gap-12">
      {(userRole === 4 ||
        userRole === 5 ||
        (userRole === 7 &&
          (bInlist === 3 || bInlist === 9 || bInlist === 10))) && (
        <div className="px-2">
          <Button
              ...
              {isLoading ? (
                ...
                sortedTickets.map((item, key) => (
                  <tr key={key} className="hover:bg-gray-100">
                    ...
                    <td className="border-b border-blue-gray-50 py-3 px-5">
                      <Typography className="text-xs font-normal text-blue-gray-600">
                        {format(new Date(item.TicketDetails?.date_created), 'yyyy-MM-dd') +
                          " " +
